...
private:
    ...
    std::queue<std::function<void(const char *,size_t)>> taskQueue_; // 这个成员变量是专门给工作线程用的，因为要解决多线程互斥问题，所以工作线程在将处理之后的数据加到自定义输出缓冲区之后要靠eventfd唤醒从线程来发送数据
...
public:
    ...
    */
    void addTaskToQueue(std::function<void(const char *,size_t)> fn);
    /*
